    ...
    protected void renderEffect(TrayColumns.TrayCursorWrapper item, int position, float intensity) {
        ...
        if (null != item) {
            ...
            if (AviaryOverlay.shouldShow(getContext(), AviaryOverlay.ID_OVERLAYS_PINCH)) {
                mOverlay = new OverlayOverlay(getContext().getBaseContext(), R.style.AviaryWidget_Overlay_Overlay);
                if (mOverlay.show()) {
                    mOverlay.setOnCloseListener(new AviaryOverlay.OnCloseListener() {
                                                   @Override
